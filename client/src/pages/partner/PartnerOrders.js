...
const PartnerOrders = () => {
  ...
  const canUpdateStatus = (item) => {
    ...
    return item.seller === user._id && ['confirmed', 'processing', 'shipped'].includes(item.status)
  }
  ...
  return (
    <div className="partner-orders-page">
      ...
      <div className="orders-header">
        ...
        <button className="btn-back" onClick={() => navigate('/partner/dashboard')}>
          ← Về Dashboard
        </button>
      </div>
      ...
      {orders.length === 0 ? (
        ...
        <div className="orders-list">
          {orders.map((order) => {
            ...
            const myItems = order.items.filter((item) => item.seller === user._id)
            const otherItems = order.items.filter((item) => item.seller !== user._id)
                      ...
                      <div className="other-items-list">
                        {otherItems.map((item) => (
                          <div key={item._id} className="order-item">
                            <img src={item.imageUrl || '/default-shoe.png'} alt={item.name} />
                            <div className="item-details">
                              <h5>{item.name}</h5>
                              <p className="seller-name">Shop: {item.sellerName}</p>
                              <div className="item-meta">
