...
const GuestChatWidget = () => {
  ...
  // Initialize socket connection for guest
  useEffect(() => {
    ...
    newSocket.on('connect', () => {
      ...
      newSocket.emit('guest:join', anonymousId)
    })
  ...
  // Select partner and create conversation
  ...
  if (showNamePrompt) {
    return (
      <div className="guest-name-prompt-overlay">
        <motion.div
          className="guest-name-prompt"
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
        >
          <h3>Xin chào! 👋</h3>
          <p>Vui lòng cho chúng tôi biết tên của bạn:</p>
          <input
            type="text"
            ...
            autoFocus
          />
          <button onClick={handleSaveName} disabled={!guestName.trim()}>
            Tiếp tục
          </button>
        </motion.div>
      </div>
    ...
    <>
      ...
      <AnimatePresence>
        {!isOpen && (
          <motion.button
            ref={buttonRef}
            className="chat-widget-button guest-chat-button"
          ...
          >
            ...
            <span className="guest-badge">Cần hỗ trợ?</span>
          </motion.button>
        )}
      </AnimatePresence>
                      ...
                      {messages.length === 0 ? (
                        ...
                        messages.map((msg) => (
                          <div
                            key={msg._id}
                            className={`message ${
                              msg.senderType === 'anonymous' ? 'sent' : 'received'
                            } ${msg.temp ? 'temp' : ''}`}
                          >
