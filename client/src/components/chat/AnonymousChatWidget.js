...
const AnonymousChatWidget = () => {
  ...
  const fetchMessages = async (convId) => {
    try {
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:5000/api'
      const response = await axios.get(`${apiUrl}/chat/conversations/${convId}/messages`, {
        headers: getHeaders(),
      })
            ...
            >
              <div className="header-left">
                {!showPartnerList && selectedPartner && (
                  <button onClick={handleBackToList} className="back-button" title="Back to list">
                    ←
                  </button>
                )}
                    ...
                    <div className="messages-container">
                      {messages.length === 0 ? (
                        <div className="empty-chat">
                          ...
                          {!user && <small>You can ask about products, prices...</small>}
                        </div>
                      ) : (
                        messages.map((msg) => (
                          <motion.div
                          ...
                          >
                            <div className="message-sender">{getSenderName(msg)}</div>
