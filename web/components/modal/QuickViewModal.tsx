...
const PLACEHOLDER_IMAGES = {
  ...
  productSmall: '/images/placeholder-product-small.svg',
}
...
const QuickViewModal = ({ product, onClose, onAddToCart, onToggleWishlist, isInWishlist }: QuickViewModalProps) => {
  ...
  const [selectedEdition, setSelectedEdition] = useState<string>('Standard') // Standard, Premium, Limited
  const [selectedSoleType, setSelectedSoleType] = useState<string>('Regular') // Regular, Air, Boost
  const [selectedBoxType, setSelectedBoxType] = useState<string>('Standard') // Standard, Premium
  const [selectedWarranty, setSelectedWarranty] = useState<string>('12') // 12, 24, 36 months
  ...
  const editionPricing: { [key: string]: number } = {
    'Standard': 0,
    'Premium': 500000, // +500k
  ...
  // Sole type pricing
  const soleTypePricing: { [key: string]: number } = {
    'Regular': 0,
    'Air Max': 800000, // +800k
  ...
  // Box type pricing
  const boxPricing: { [key: string]: number } = {
    'Standard': 0,
  ...
  // Warranty pricing
  const warrantyPricing: { [key: number]: number } = {
    12: 0,
  ...
  const genderOptions = useMemo((): string[] => {
    ...
    const genders = [
      ...new Set(product.variants.map((v: any) => v.specifications?.gender || v.gender)),
  ...
  const availableColors = useMemo(() => {
    ...
    const colors = [...new Set(product.variants.map((v: any) => v.specifications?.color || v.color))]
  ...
  useEffect(() => {
    ...
    if (matchedVariant) {
      ...
      // Silently handle missing variant - don't spam console
    }
  ...
  const finalPrice =
    currentPrice +
    (editionPricing[selectedEdition] || 0) +
    (soleTypePricing[selectedSoleType] || 0) +
    (boxPricing[selectedBoxType] || 0) +
    (warrantyPricing[Number(selectedWarranty)] || 0) +
        ...
        <div className="quickview-body">
          ...
          <div className="quickview-left">
            ...
            <div className="quickview-main-image">
              <img
                ...
                onError={(e) => ((e.target as HTMLImageElement).src = PLACEHOLDER_IMAGES.product)}
              ...
              <div className="quickview-thumbnails">
                ...
                >
                  <img
                    ...
                    alt={displayProduct.name}
                    onError={(e) => ((e.target as HTMLImageElement).src = PLACEHOLDER_IMAGES.productSmall)}
                  />
                </div>
          ...
          <div className="quickview-right">
              ...
              <div style={{ marginBottom: '25px' }}>
                ...
                <div style={{ position: 'relative', paddingTop: '10px' }}>
                  ...
                  >
                    {[34, 36, 38, 40, 42, 44].map((size) => (
                      <span
                        ...
                        style={{
                          fontSize: '11px',
                          color: parseFloat(selectedSize || '0') === size ? '#6366f1' : '#9ca3af',
                          fontWeight: parseFloat(selectedSize || '0') === size ? '700' : '500',
                        }}
                      >
                        {size}
                      </span>
                  ...
                  <input
                    ...
                    max="44"
                    step="0.5"
                    value={parseFloat(selectedSize || '36') || 36}
                    onChange={(e) => {
                        ...
                        onClick={() => {
                          ...
                          if (isAvailable) {
                            const availableSizesForColor = product.variants
                              ...
                              .map((v: any) => v.specifications?.size || v.size)
                  ...
                  {Object.keys(warrantyPricing).map((warranty) => (
                    ...
                    >
                      ...
                      <div style={{ fontSize: '11px', color: '#6b7280', marginTop: '2px' }}>
                        {warrantyPricing[Number(warranty)] === 0
                          ? 'Miễn phí'
                          : `+${(warrantyPricing[Number(warranty)] / 1000).toFixed(0)}k`}
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <div className="quickview-actions">
              ...
              <Link
                href={`/product/${displayProduct._id}`}
