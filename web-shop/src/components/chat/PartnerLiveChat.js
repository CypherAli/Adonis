...
import { FiMessageCircle, FiX, FiSend, FiMinimize2, FiUser, FiClock } from 'react-icons/fi'
...
const PartnerLiveChat = () => {
  ...
  const loadCustomers = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/chat/partner/${partnerId}/customers`, {
        headers: {
  ...
  return (
    <>
      ...
      >
        ...
        {getTotalUnread() > 0 && <span className="unread-badge">{getTotalUnread()}</span>}
            ...
            <div className="partner-chat-header">
              <div className="header-info">
                ...
                <div>
                  ...
                  <span className="header-status">{isConnected ? '● Online' : '○ Offline'}</span>
                </div>
              </div>
                      ...
                      {messages.length === 0 ? (
                        ...
                        messages.map((msg) => {
                          const isPartnerMessage =
                            msg.senderType === 'user' && msg.sender?.role === 'partner'
