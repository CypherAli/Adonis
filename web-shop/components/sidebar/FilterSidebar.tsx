...
interface FilterSidebarProps {
  tempFilters: any
  handleTempFilterChange: (filterName: string, value: any) => void
  toggleArrayFilter: (filterName: string, value: string) => void
...
const FilterSidebar = ({
  ...
  const brandData: { [key: string]: { count: number; icon?: React.ReactNode } } = {
    'Nike': { count: 12 },
    'Adidas': { count: 15 },
    'Puma': { count: 8 },
    'Converse': { count: 6 },
  ...
  return (
    <aside className="sidebar">
      <div className="filter-section-new">
        ...
        <div className="filter-header-new">
          ...
          {activeFiltersCount > 0 && (
            <span className="active-filters-badge-new">{activeFiltersCount}</span>
          )}
        </div>
        ...
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('brand')}>
              ...
              <div className="filter-brand-list">
                {brands.map((brand) => {
                  ...
                  return (
                    ...
                    >
                      <div className="brand-icon-wrapper">{data.icon}</div>
                      <div className="brand-info">
                        <span className="brand-name">{brand}</span>
                        <span className="brand-count">{data.count}</span>
                      </div>
                      {isSelected && <div className="check-badge-new">✓</div>}
                    </div>
                  )
                })}
              </div>
        ...
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('size')}>
            <div className="header-left">
              <span>SIZE</span>
            </div>
        ...
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('color')}>
            <div className="header-left">
              <span>COLOR</span>
            </div>
        ...
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('material')}>
            <div className="header-left">
              <span>MATERIAL</span>
            </div>
            ...
            <div className="filter-options-grid">
              {materialOptions &&
                materialOptions.map((material) => {
                  const isSelected =
                    tempFilters.materials && tempFilters.materials.includes(material)
                  return (
                    <div
                      key={material}
                      className={`option-chip ${isSelected ? 'selected' : ''}`}
        ...
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('price')}>
            <div className="header-left">
                  ...
                  <input
                    ...
                    value={
                      tempFilters.minPrice
                        ? `${parseInt(tempFilters.minPrice).toLocaleString()}đ`
                        : ''
                    }
                    onChange={(e) => {
                  ...
                  <input
                    ...
                    value={
                      tempFilters.maxPrice
                        ? `${parseInt(tempFilters.maxPrice).toLocaleString()}đ`
                        : ''
                    }
                    onChange={(e) => {
                      const value = e.target.value.replace(/[^\d]/g, '')
        ...
        {/* Sort By - Dropdown */}
        <div className="filter-group-new">
          <div className="filter-group-header-new" onClick={() => toggleSection('sort')}>
            <div className="header-left">
              <span>SORT BY</span>
            </div>
      ...
      <div className="filter-actions-fixed">
        <button className="apply-btn-new" onClick={handleApplyFilters}>
          APPLY
        </button>

        <button className="clear-btn-new" onClick={handleClearFilters}>
          CLEAR
        </button>
      </div>
    </aside>
